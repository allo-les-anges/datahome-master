  ...
  if (loading && !agency) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">{t('common.loading') || "Chargement..."}</p>
      </div>
    );
  }

  // Styles inline pour le formulaire
  const inputStyle = {
    width: '100%',
    padding: '16px',
    borderRadius: '12px',
    ...
    <div style={{ backgroundColor: '#ffffff', color: '#0f172a' }}>
      <main>
        ...
        <section className="max-w-[1400px] mx-auto px-6 py-20 grid grid-cols-1 xl:grid-cols-12 gap-16">
          ...
          <div className="xl:col-span-7 space-y-10">
            <div className="flex items-center gap-4 border-l-4 pl-6" style={{ borderColor: brandColor }}>
              <h2 className="text-3xl font-serif italic uppercase tracking-widest">
                {agency?.about_title || t('contact.teamTitle') || "Notre Équipe"}
              </h2>
            </div>
          ...
          {/* Formulaire de contact - Version iPhone */}
                ...
                {status === "success" ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
                    ...
                    <p className="text-xs text-slate-400">
                      {t('contact.success_message') || "Nous vous répondrons dans les plus brefs délais."}
                    </p>
                    <button 
                  ...
                  <>
                    <h3 className="text-lg font-serif italic text-white mb-6 text-center tracking-widest uppercase">
                      {t('contact.directContact') || "Contact direct"}
                    </h3>
                    ...
                    <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-3">
                      ...
                      {status === "error" && (
                        <div className="flex items-center justify-center gap-2 text-red-400 text-[9px] mt-2 uppercase tracking-widest">
                          <AlertCircle size={12} />
                          {t('contact.error_message') || "Erreur lors de l'envoi"}
                        </div>
                      )}
                    </form>
                  </>
