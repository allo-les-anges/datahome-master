...
export default function ContactPage({ params }: ContactPageProps) {
  ...
  // Synchronisation sécurisée du slug
  useEffect(() => {
    const currentSlug = resolvedParams.slug;
    // On ne déclenche l'appel que si le slug de l'URL est différent de celui chargé en mémoire
    if (currentSlug && agency?.subdomain !== currentSlug && !loading) {
      setAgencyBySlug(currentSlug);
    }
  }, [resolvedParams.slug, agency?.subdomain, loading]); // Retrait de setAgencyBySlug des dépendances si non-callback

  // Parsing des données avec useMemo pour éviter de recalculer à chaque rendu
  const { contactInfo, displayTeam, brandColor, buttonColor, selectedFont } = useMemo(() => {
    ...
    return {
      ...
      selectedFont: agency?.font_family || 'Montserrat'
    };
  }, [agency]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white text-center">
        <div>
           ...
           <button onClick={() => window.location.reload()} className="text-[10px] font-black uppercase border-b border-black">Réessayer</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900" style={{ fontFamily: selectedFont }}>
      <Navbar />
      <main>
        {/* Hero */}
        ...
        <section className="max-w-[1400px] mx-auto px-6 py-20 grid grid-cols-1 xl:grid-cols-12 gap-16">
          <div className="xl:col-span-7 space-y-10">
            <div className="flex items-center gap-4 border-l-4 pl-6" style={{ borderColor: brandColor }}>
              <h2 className="text-3xl font-serif italic uppercase tracking-widest">{agency?.about_title || "L'Équipe"}</h2>
            </div>
            
            <div className="grid gap-8">
              {displayTeam.map((member: any, idx: number) => (
                <motion.div key={idx} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} className="p-6 rounded-[2rem] border border-slate-100 bg-slate-50/50 flex flex-col md:flex-row gap-8 items-center shadow-sm">
            ...
            <div className="relative w-full max-w-[380px] aspect-[9/18.5] rounded-[3.5rem] bg-slate-900 p-3 shadow-2xl border-[8px] border-slate-800">
              <div className="relative w-full h-full rounded-[2.8rem] overflow-hidden bg-slate-950 flex flex-col p-8 pt-12">
                ...
                <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-3">
                  <input required className="w-full p-4 rounded-xl bg-white/5 border border-white/10 text-[10px] text-white outline-none focus:border-white/30" placeholder="VOTRE NOM" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} />
                  <input required type="email" className="w-full p-4 rounded-xl bg-white/5 border border-white/10 text-[10px] text-white outline-none focus:border-white/30" placeholder="VOTRE EMAIL" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} />
                  <textarea required rows={4} className="w-full p-4 rounded-xl bg-white/5 border border-white/10 text-[10px] text-white outline-none focus:border-white/30 resize-none" placeholder="VOTRE MESSAGE..." value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} />
                  <button type="submit" disabled={isSubmitting} style={{ backgroundColor: buttonColor }} className="mt-auto w-full py-4 rounded-xl text-black font-black text-[10px] tracking-[0.3em] hover:brightness-110 active:scale-95 transition-all shadow-xl disabled:opacity-50 flex items-center justify-center gap-2">
