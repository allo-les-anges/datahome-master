...
export default async function TermsPage({
  ...
  const content: string = agency.terms_content || translations.legal?.terms_default || "";
    ...
    >
      ...
      <header className="relative overflow-hidden pt-20 pb-16 px-6">
        <div
          ...
          style={{ backgroundImage: `radial-gradient(circle at 70% 50%, ${brandColor} 0%, transparent 60%)` }}
        />
        <div className="max-w-5xl mx-auto relative z-10">
          ...
          <div className="flex items-center gap-3 ml-20">
            <Scale size={12} className="text-white/20" />
            <p className="text-[10px] uppercase tracking-[0.2em] font-black text-white/20">
              Document contractuel · {agency.agency_name}
            </p>
          </div>
        </div>
            ...
            {sections.slice(0, 6).map((section: string, i: number) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <span className="text-[9px] uppercase tracking-[0.15em] font-bold text-white/25 group-hover:text-white/50 transition-colors duration-200 line-clamp-1">
                    {title}
                  </span>
                </a>
              );
                ...
                {sections.map((section: string, i: number) => {
                  ...
                  return (
                    <div key={i} id={`section-${i}`} className="scroll-mt-24">
                      ...
                      {i < sections.length - 1 && (
                        <div className="mt-10 h-px" style={{ background: "rgba(255,255,255,0.04)" }} />
                      )}
                    </div>
                  );
                })}
